...
public record DNAExtractorRecipeInput(List<ItemStack> inputs) implements RecipeInput {

    public DNAExtractorRecipeInput {
        Objects.requireNonNull(inputs, "inputs");
        if (inputs.size() != 2) {
            throw new IllegalArgumentException("DNAExtractorRecipeInput requires exactly 2 input stacks, got " + inputs.size());
        }
