...
public record DNAAnalyzerRecipe(NonNullList<Ingredient> inputs, ItemStack output
                                 , java.util.Map<ResourceLocation, Integer> weights // optional per-output weights
) implements Recipe<DNAAnalyzerRecipeInput> {

    public DNAAnalyzerRecipe(Ingredient first, Ingredient second, ItemStack output) {
        this(NonNullList.create(), output, java.util.Map.of());
        this.inputs.add(first);
        this.inputs.add(second);
    }

    public DNAAnalyzerRecipe(NonNullList<Ingredient> inputs, ItemStack output) {
        this(inputs, output, java.util.Map.of());
    }

    // Convenience for codec constructor
    public DNAAnalyzerRecipe(NonNullList<Ingredient> inputs, ItemStack output, java.util.Map<ResourceLocation, Integer> weights) {
        this.inputs = inputs;
        this.output = output;
    ...
    public @NotNull RecipeType<?> getType() {
        return ModRecipes.DNA_ANALYZER_RECIPE_TYPE.get();
    }
    ...
    @Override
    public boolean matches(@NotNull DNAAnalyzerRecipeInput recipeInput, Level level) {
    ...
    @Override
    public @NotNull ItemStack assemble(@NotNull DNAAnalyzerRecipeInput recipeInput, HolderLookup.@NotNull Provider provider) {
        return output.copy();
    }
    ...
    public static class Serializer implements RecipeSerializer<DNAAnalyzerRecipe> {
        ...
        public static final MapCodec<DNAAnalyzerRecipe> CODEC = RecordCodecBuilder.mapCodec(instance ->
                instance.group(
                        Ingredient.CODEC_NONEMPTY.listOf().fieldOf("ingredients")
                                .flatXmap(list -> {
                                            if (list.size() != 2) {
                                                return DataResult.error(() -> "DNAAnalyzerRecipe requires exactly 2 ingredients, got " + list.size());
                                            }
                        ...
                        WEIGHTS_CODEC.optionalFieldOf("weights", java.util.Map.of())
                                .forGetter(DNAAnalyzerRecipe::weights)
                ).apply(instance, DNAAnalyzerRecipe::new)
        );
        ...
        public static final StreamCodec<RegistryFriendlyByteBuf, DNAAnalyzerRecipe> STREAM_CODEC = StreamCodec.of(
                ...
                buf -> {
                    ...
                    if (decodedInputs.size() != 2) {
                        throw new IllegalArgumentException("DNAAnalyzerRecipe requires exactly 2 ingredients in stream, got " + decodedInputs.size());
                    }
                    ...
                    return new DNAAnalyzerRecipe(decodedInputs, result, weights);
                }
        );

        @Override
        public @NotNull MapCodec<DNAAnalyzerRecipe> codec() {
            return CODEC;
        }

        @Override
        public @NotNull StreamCodec<RegistryFriendlyByteBuf, DNAAnalyzerRecipe> streamCodec() {
            return STREAM_CODEC;
        }
    }
}
