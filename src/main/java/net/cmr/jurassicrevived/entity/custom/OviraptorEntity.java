...
public class OviraptorEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(OviraptorEntity.class, EntityDataSerializers.INT);
    ...
    public OviraptorEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return OviraptorEntity.this.isBaby() && super.canUse();
            }
        });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.OVIRAPTOR.get().create(pLevel);
        if (child instanceof OviraptorEntity baby) {
            OviraptorVariant randomVariant = Util.getRandom(OviraptorVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(OviraptorEntity.this.isSprinting() ? RawAnimation.begin().then("anim.oviraptor.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.oviraptor.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.oviraptor.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.oviraptor.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.oviraptor.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.OVIRAPTOR_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public OviraptorVariant getVariant() {
        return OviraptorVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(OviraptorVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof OviraptorEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        OviraptorVariant variant = Util.getRandom(OviraptorVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.OVIRAPTOR_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.OVIRAPTOR_DEATH.get();
    }
}