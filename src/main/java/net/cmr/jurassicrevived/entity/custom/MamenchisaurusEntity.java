...
public class MamenchisaurusEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(MamenchisaurusEntity.class, EntityDataSerializers.INT);
    ...
    public MamenchisaurusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        ...
        public boolean canUse() {
            return MamenchisaurusEntity.this.isBaby() && super.canUse();
        }
    });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.MAMENCHISAURUS.get().create(pLevel);
        if (child instanceof MamenchisaurusEntity baby) {
            MamenchisaurusVariant randomVariant = Util.getRandom(MamenchisaurusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(MamenchisaurusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.mamenchisaurus.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.mamenchisaurus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.mamenchisaurus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.mamenchisaurus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.mamenchisaurus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.MAMENCHISAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public MamenchisaurusVariant getVariant() {
        return MamenchisaurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(MamenchisaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof MamenchisaurusEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        MamenchisaurusVariant variant = Util.getRandom(MamenchisaurusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.MAMENCHISAURUS_HURT.get();
    }
    ...
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.MAMENCHISAURUS_DEATH.get();
    }
}