...
import net.cmr.jurassicrevived.entity.client.TapejaraVariant;
...
public class TapejaraEntity extends Animal implements GeoEntity, FlyingAnimal {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(TapejaraEntity.class, EntityDataSerializers.INT);
    ...
    public TapejaraEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return TapejaraEntity.this.isBaby() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(7, new WaterAvoidingRandomStrollGoal(this, 1.0) {
            @Override
            public boolean canUse() {
                return TapejaraEntity.this.onGround() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(8, new WaterAvoidingRandomStrollGoal(this, 1.0, 20) {
            @Override
            public boolean canUse() {
                boolean isFlying = !TapejaraEntity.this.onGround();
                // If flying, keep flying. If on ground, small chance (1/400 ticks) to take off.
                return (isFlying || TapejaraEntity.this.getRandom().nextInt(400) == 0) && super.canUse();
            }

            @Override
            protected Vec3 getPosition() {
                Vec3 pos = TapejaraEntity.this.position();
                RandomSource random = TapejaraEntity.this.getRandom();
                ...
                int groundY = TapejaraEntity.this.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int)x, (int)z);
                double y;

                if (TapejaraEntity.this.onGround()) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.TAPEJARA.get().create(pLevel);
        if (child instanceof TapejaraEntity baby) {
            TapejaraVariant randomVariant = Util.getRandom(TapejaraVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (!TapejaraEntity.this.onGround()) {
                return state.setAndContinue(RawAnimation.begin().then("anim.tapejara.fly", Animation.LoopType.LOOP));
            }

            if (state.isMoving())
                return state.setAndContinue(TapejaraEntity.this.isSprinting() ? RawAnimation.begin().then("anim.tapejara.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.tapejara.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.tapejara.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.tapejara.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.tapejara.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.TAPEJARA_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public TapejaraVariant getVariant() {
        return TapejaraVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(TapejaraVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof TapejaraEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        TapejaraVariant variant = Util.getRandom(TapejaraVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.TAPEJARA_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.TAPEJARA_DEATH.get();
    }
}