...
import net.cmr.jurassicrevived.entity.client.StyracosaurusVariant;
...
public class StyracosaurusEntity extends Animal implements GeoEntity {
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.STYRACOSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.STYRACOSAURUS_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.STYRACOSAURUS_DEATH.get();
    }
}