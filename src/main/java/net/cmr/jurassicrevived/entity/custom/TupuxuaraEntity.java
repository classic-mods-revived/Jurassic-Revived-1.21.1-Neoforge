...
public class TupuxuaraEntity extends Animal implements GeoEntity, FlyingAnimal {
    ...
    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(TupuxuaraEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.TUPUXUARA.get().create(pLevel);
        if (child instanceof TupuxuaraEntity baby) {
            TupuxuaraVariant randomVariant = Util.getRandom(TupuxuaraVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            ...
            if (state.isMoving())
                return state.setAndContinue(TupuxuaraEntity.this.isSprinting() ? RawAnimation.begin().then("anim.tupuxuara.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.tupuxuara.walk", Animation.LoopType.LOOP));
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.TUPUXUARA_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public TupuxuaraVariant getVariant() {
        return TupuxuaraVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(TupuxuaraVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof TupuxuaraEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        TupuxuaraVariant variant = Util.getRandom(TupuxuaraVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.TUPUXUARA_HURT.get();
    }
