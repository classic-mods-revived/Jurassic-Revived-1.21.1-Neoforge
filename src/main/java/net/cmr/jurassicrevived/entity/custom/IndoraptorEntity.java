...
public class IndoraptorEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(IndoraptorEntity.class, EntityDataSerializers.INT);
    ...
    public IndoraptorEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        ...
        public boolean canUse() {
            return IndoraptorEntity.this.isBaby() && super.canUse();
        }
    });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.INDORAPTOR.get().create(pLevel);
        if (child instanceof IndoraptorEntity baby) {
    ...
    public boolean doHurtTarget(Entity target) {
        ...
        if (!level().isClientSide && hit && target instanceof LivingEntity) {
            if (this.level() instanceof ServerLevel serverLevel) {
                ...
                this.playSound(ModSounds.INDORAPTOR_ATTACK.get(), 1.0F, 1.0F);
            }
        }
        return hit;
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(IndoraptorEntity.this.isSprinting() ? RawAnimation.begin().then("anim.indoraptor.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.indoraptor.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.indoraptor.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.indoraptor.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.indoraptor.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.INDORAPTOR_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public IndoraptorVariant getVariant() {
        return IndoraptorVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(IndoraptorVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof IndoraptorEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        IndoraptorVariant variant = Util.getRandom(IndoraptorVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.INDORAPTOR_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.INDORAPTOR_DEATH.get();
    }
}