...
public class OrnithomimusEntity extends Animal implements GeoEntity {
    ...
    public OrnithomimusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.ORNITHOMIMUS.get().create(pLevel);
        if (child instanceof OrnithomimusEntity baby) {
            OrnithomimusVariant randomVariant = Util.getRandom(OrnithomimusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(OrnithomimusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.ornithomimus.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.ornithomimus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.ornithomimus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.ornithomimus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.ornithomimus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.ORNITHOMIMUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public OrnithomimusVariant getVariant() {
        return OrnithomimusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(OrnithomimusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof OrnithomimusEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        OrnithomimusVariant variant = Util.getRandom(OrnithomimusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.ORNITHOMIMUS_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.ORNITHOMIMUS_DEATH.get();
    }
}