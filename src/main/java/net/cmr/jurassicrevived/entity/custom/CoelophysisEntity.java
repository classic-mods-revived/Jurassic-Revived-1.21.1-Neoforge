...
public class CoelophysisEntity extends Animal implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(CoelophysisEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.COELOPHYSIS.get().create(pLevel);
        if (child instanceof CoelophysisEntity baby) {
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.COELOPHYSIS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public CoelophysisVariant getVariant() {
        return CoelophysisVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(CoelophysisVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof CoelophysisEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        CoelophysisVariant variant = Util.getRandom(CoelophysisVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.COELOPHYSIS_HURT.get();
    }
