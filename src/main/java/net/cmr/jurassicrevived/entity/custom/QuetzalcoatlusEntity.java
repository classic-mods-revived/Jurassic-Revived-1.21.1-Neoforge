...
import net.cmr.jurassicrevived.entity.client.QuetzalcoatlusVariant;
...
public class QuetzalcoatlusEntity extends Animal implements GeoEntity, FlyingAnimal {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(QuetzalcoatlusEntity.class, EntityDataSerializers.INT);
    ...
    public QuetzalcoatlusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return QuetzalcoatlusEntity.this.isBaby() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(7, new WaterAvoidingRandomStrollGoal(this, 1.0) {
            @Override
            public boolean canUse() {
                return QuetzalcoatlusEntity.this.onGround() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(8, new WaterAvoidingRandomStrollGoal(this, 1.0, 20) {
            @Override
            public boolean canUse() {
                boolean isFlying = !QuetzalcoatlusEntity.this.onGround();
                // If flying, keep flying. If on ground, small chance (1/400 ticks) to take off.
                return (isFlying || QuetzalcoatlusEntity.this.getRandom().nextInt(400) == 0) && super.canUse();
            }

            @Override
            protected Vec3 getPosition() {
                Vec3 pos = QuetzalcoatlusEntity.this.position();
                RandomSource random = QuetzalcoatlusEntity.this.getRandom();
                ...
                int groundY = QuetzalcoatlusEntity.this.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int)x, (int)z);
                double y;

                if (QuetzalcoatlusEntity.this.onGround()) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.QUETZALCOATLUS.get().create(pLevel);
        if (child instanceof QuetzalcoatlusEntity baby) {
            QuetzalcoatlusVariant randomVariant = Util.getRandom(QuetzalcoatlusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (!QuetzalcoatlusEntity.this.onGround()) {
                return state.setAndContinue(RawAnimation.begin().then("anim.quetzalcoatlus.fly", Animation.LoopType.LOOP));
            }

            if (state.isMoving())
                return state.setAndContinue(QuetzalcoatlusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.quetzalcoatlus.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.quetzalcoatlus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.quetzalcoatlus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.quetzalcoatlus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.quetzalcoatlus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.QUETZALCOATLUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public QuetzalcoatlusVariant getVariant() {
        return QuetzalcoatlusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(QuetzalcoatlusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof QuetzalcoatlusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        QuetzalcoatlusVariant variant = Util.getRandom(QuetzalcoatlusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.QUETZALCOATLUS_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.QUETZALCOATLUS_DEATH.get();
    }
}