...
public class InostranceviaEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(InostranceviaEntity.class, EntityDataSerializers.INT);
    ...
    public InostranceviaEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        ...
        public boolean canUse() {
            return InostranceviaEntity.this.isBaby() && super.canUse();
        }
    });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.INOSTRANCEVIA.get().create(pLevel);
        if (child instanceof InostranceviaEntity baby) {
            InostranceviaVariant randomVariant = Util.getRandom(InostranceviaVariant.values(), this.random);
    ...
    public boolean doHurtTarget(Entity target) {
        ...
        if (!level().isClientSide && hit && target instanceof LivingEntity) {
            if (this.level() instanceof ServerLevel serverLevel) {
                ...
                this.playSound(ModSounds.INOSTRANCEVIA_ATTACK.get(), 1.0F, 1.0F);
            }
        }
        return hit;
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(InostranceviaEntity.this.isSprinting() ? RawAnimation.begin().then("anim.inostrancevia.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.inostrancevia.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.inostrancevia.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.inostrancevia.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.inostrancevia.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.INOSTRANCEVIA_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public InostranceviaVariant getVariant() {
        return InostranceviaVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(InostranceviaVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof InostranceviaEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        InostranceviaVariant variant = Util.getRandom(InostranceviaVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.INOSTRANCEVIA_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.INOSTRANCEVIA_DEATH.get();
    }
}