...
public class DryosaurusEntity extends Animal implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(DryosaurusEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.DRYOSAURUS.get().create(pLevel);
        if (child instanceof DryosaurusEntity baby) {
            DryosaurusVariant randomVariant = Util.getRandom(DryosaurusVariant.values(), this.random);
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.DRYOSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public DryosaurusVariant getVariant() {
        return DryosaurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(DryosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof DryosaurusEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        DryosaurusVariant variant = Util.getRandom(DryosaurusVariant.values(), this.random);
