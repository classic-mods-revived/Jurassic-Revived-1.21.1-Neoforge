...
public class CearadactylusEntity extends Animal implements GeoEntity, FlyingAnimal {
    ...
    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(CearadactylusEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.CEARADACTYLUS.get().create(pLevel);
        if (child instanceof CearadactylusEntity baby) {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            ...
            if (state.isMoving())
                return state.setAndContinue(CearadactylusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.cearadactylus.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.cearadactylus.walk", Animation.LoopType.LOOP));
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.CEARADACTYLUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    private void setVariant(CearadactylusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof CearadactylusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        CearadactylusVariant variant = Util.getRandom(CearadactylusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.CEARADACTYLUS_HURT.get();
    }
