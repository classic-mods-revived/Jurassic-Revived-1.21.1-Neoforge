...
import net.cmr.jurassicrevived.entity.client.RugopsVariant;
...
public class RugopsEntity extends Animal implements GeoEntity {
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.RUGOPS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.RUGOPS_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.RUGOPS_DEATH.get();
    }
}