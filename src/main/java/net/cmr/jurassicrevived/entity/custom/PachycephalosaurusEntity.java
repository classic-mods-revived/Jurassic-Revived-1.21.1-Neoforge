...
public class PachycephalosaurusEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(PachycephalosaurusEntity.class, EntityDataSerializers.INT);
    ...
    public PachycephalosaurusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
        ...
        public boolean canUse() {
            return PachycephalosaurusEntity.this.isBaby() && super.canUse();
        }
    });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.PACHYCEPHALOSAURUS.get().create(pLevel);
        if (child instanceof PachycephalosaurusEntity baby) {
            PachycephalosaurusVariant randomVariant = Util.getRandom(PachycephalosaurusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(PachycephalosaurusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.pachycephalosaurus.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.pachycephalosaurus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.pachycephalosaurus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.pachycephalosaurus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.pachycephalosaurus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.PACHYCEPHALOSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public PachycephalosaurusVariant getVariant() {
        return PachycephalosaurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(PachycephalosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof PachycephalosaurusEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        PachycephalosaurusVariant variant = Util.getRandom(PachycephalosaurusVariant.values(), this.random);
