...
import net.cmr.jurassicrevived.entity.client.UtahraptorVariant;
...
public class UtahraptorEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(UtahraptorEntity.class, EntityDataSerializers.INT);
    ...
    protected void registerGoals() {
        ...
        public boolean canUse() {
            return UtahraptorEntity.this.isBaby() && super.canUse();
        }
    });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.UTAHRAPTOR.get().create(pLevel);
        if (child instanceof UtahraptorEntity baby) {
            UtahraptorVariant randomVariant = Util.getRandom(UtahraptorVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(UtahraptorEntity.this.isSprinting() ? RawAnimation.begin().then("anim.utahraptor.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.utahraptor.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.utahraptor.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.utahraptor.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.utahraptor.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.UTAHRAPTOR_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public UtahraptorVariant getVariant() {
        return UtahraptorVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(UtahraptorVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof UtahraptorEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        UtahraptorVariant variant = Util.getRandom(UtahraptorVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.UTAHRAPTOR_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.UTAHRAPTOR_DEATH.get();
    }
}