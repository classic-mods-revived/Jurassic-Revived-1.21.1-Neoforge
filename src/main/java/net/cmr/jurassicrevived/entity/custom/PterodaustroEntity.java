...
import net.cmr.jurassicrevived.entity.client.PterodaustroVariant;
...
public class PterodaustroEntity extends Animal implements GeoEntity, FlyingAnimal {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(PterodaustroEntity.class, EntityDataSerializers.INT);
    ...
    public PterodaustroEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return PterodaustroEntity.this.isBaby() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(7, new WaterAvoidingRandomStrollGoal(this, 1.0) {
            @Override
            public boolean canUse() {
                return PterodaustroEntity.this.onGround() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(8, new WaterAvoidingRandomStrollGoal(this, 1.0, 20) {
            @Override
            public boolean canUse() {
                boolean isFlying = !PterodaustroEntity.this.onGround();
                // If flying, keep flying. If on ground, small chance (1/400 ticks) to take off.
                return (isFlying || PterodaustroEntity.this.getRandom().nextInt(400) == 0) && super.canUse();
            }

            @Override
            protected Vec3 getPosition() {
                Vec3 pos = PterodaustroEntity.this.position();
                RandomSource random = PterodaustroEntity.this.getRandom();
                ...
                int groundY = PterodaustroEntity.this.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int)x, (int)z);
                double y;

                if (PterodaustroEntity.this.onGround()) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.PTERODAUSTRO.get().create(pLevel);
        if (child instanceof PterodaustroEntity baby) {
            PterodaustroVariant randomVariant = Util.getRandom(PterodaustroVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (!PterodaustroEntity.this.onGround()) {
                return state.setAndContinue(RawAnimation.begin().then("anim.pterodaustro.fly", Animation.LoopType.LOOP));
            }

            if (state.isMoving())
                return state.setAndContinue(PterodaustroEntity.this.isSprinting() ? RawAnimation.begin().then("anim.pterodaustro.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.pterodaustro.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.pterodaustro.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.pterodaustro.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.pterodaustro.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.PTERODAUSTRO_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public PterodaustroVariant getVariant() {
        return PterodaustroVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(PterodaustroVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof PterodaustroEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        PterodaustroVariant variant = Util.getRandom(PterodaustroVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.PTERODAUSTRO_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.PTERODAUSTRO_DEATH.get();
    }
}