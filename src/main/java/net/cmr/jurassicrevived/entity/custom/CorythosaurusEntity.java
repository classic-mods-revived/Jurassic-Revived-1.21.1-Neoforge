...
public class CorythosaurusEntity extends Animal implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(CorythosaurusEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.CORYTHOSAURUS.get().create(pLevel);
        if (child instanceof CorythosaurusEntity baby) {
            CorythosaurusVariant randomVariant = Util.getRandom(CorythosaurusVariant.values(), this.random);
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.CORYTHOSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public CorythosaurusVariant getVariant() {
        return CorythosaurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(CorythosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof CorythosaurusEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        CorythosaurusVariant variant = Util.getRandom(CorythosaurusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.CORYTHOSAURUS_HURT.get();
    }
