...
import net.cmr.jurassicrevived.entity.client.TherizinosaurusVariant;
    ...
    public boolean doHurtTarget(Entity target) {
        ...
        if (!level().isClientSide && hit && target instanceof LivingEntity) {
            if (this.level() instanceof ServerLevel serverLevel) {
                ...
                this.playSound(ModSounds.TAIL_STRIKE.get(), 1.0F, 1.0F);
            }
        }
        return hit;
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.THERIZINOSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
