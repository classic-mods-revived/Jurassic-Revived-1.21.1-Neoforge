...
import net.cmr.jurassicrevived.entity.client.TyrannosaurusRexVariant;
    ...
    public boolean doHurtTarget(Entity target) {
        ...
        if (!level().isClientSide && hit && target instanceof LivingEntity) {
            if (this.level() instanceof ServerLevel serverLevel) {
                ...
                this.playSound(ModSounds.TYRANNOSAURUS_REX_ATTACK.get(), 1.0F, 1.0F);
            }
        }
        return hit;
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.TYRANNOSAURUS_REX_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
