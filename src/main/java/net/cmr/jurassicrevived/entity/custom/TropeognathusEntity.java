...
import net.cmr.jurassicrevived.entity.client.TropeognathusVariant;
...
public class TropeognathusEntity extends Animal implements GeoEntity, FlyingAnimal {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(TropeognathusEntity.class, EntityDataSerializers.INT);
    ...
    public TropeognathusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return TropeognathusEntity.this.isBaby() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(7, new WaterAvoidingRandomStrollGoal(this, 1.0) {
            @Override
            public boolean canUse() {
                return TropeognathusEntity.this.onGround() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(8, new WaterAvoidingRandomStrollGoal(this, 1.0, 20) {
            @Override
            public boolean canUse() {
                boolean isFlying = !TropeognathusEntity.this.onGround();
                // If flying, keep flying. If on ground, small chance (1/400 ticks) to take off.
                return (isFlying || TropeognathusEntity.this.getRandom().nextInt(400) == 0) && super.canUse();
            }

            @Override
            protected Vec3 getPosition() {
                Vec3 pos = TropeognathusEntity.this.position();
                RandomSource random = TropeognathusEntity.this.getRandom();
                ...
                int groundY = TropeognathusEntity.this.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int)x, (int)z);
                double y;

                if (TropeognathusEntity.this.onGround()) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.TROPEOGNATHUS.get().create(pLevel);
        if (child instanceof TropeognathusEntity baby) {
            TropeognathusVariant randomVariant = Util.getRandom(TropeognathusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (!TropeognathusEntity.this.onGround()) {
                return state.setAndContinue(RawAnimation.begin().then("anim.tropeognathus.fly", Animation.LoopType.LOOP));
            }

            if (state.isMoving())
                return state.setAndContinue(TropeognathusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.tropeognathus.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.tropeognathus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.tropeognathus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.tropeognathus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.tropeognathus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.TROPEOGNATHUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public TropeognathusVariant getVariant() {
        return TropeognathusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(TropeognathusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof TropeognathusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        TropeognathusVariant variant = Util.getRandom(TropeognathusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.TROPEOGNATHUS_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.TROPEOGNATHUS_DEATH.get();
    }
}