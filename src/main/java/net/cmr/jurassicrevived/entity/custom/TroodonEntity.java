...
import net.cmr.jurassicrevived.entity.client.TroodonVariant;
...
public class TroodonEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(TroodonEntity.class, EntityDataSerializers.INT);
    ...
    public TroodonEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        ...
        public boolean canUse() {
            return TroodonEntity.this.isBaby() && super.canUse();
        }
    });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.TROODON.get().create(pLevel);
        if (child instanceof TroodonEntity baby) {
            TroodonVariant randomVariant = Util.getRandom(TroodonVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(TroodonEntity.this.isSprinting() ? RawAnimation.begin().then("anim.troodon.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.troodon.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.troodon.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.troodon.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.troodon.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.TROODON_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public TroodonVariant getVariant() {
        return TroodonVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(TroodonVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof TroodonEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        TroodonVariant variant = Util.getRandom(TroodonVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.TROODON_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.TROODON_DEATH.get();
    }
}