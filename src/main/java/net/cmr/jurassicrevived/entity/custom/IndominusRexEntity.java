...
public class IndominusRexEntity extends Animal implements GeoEntity {
    ...
    public boolean doHurtTarget(Entity target) {
        ...
        if (!level().isClientSide && hit && target instanceof LivingEntity) {
            if (this.level() instanceof ServerLevel serverLevel) {
                ...
                this.playSound(ModSounds.INDOMINUS_REX_ATTACK.get(), 1.0F, 1.0F);
            }
        }
        return hit;
    }
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(IndominusRexEntity.this.isSprinting() ? RawAnimation.begin().then("anim.indominus_rex.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.indominus_rex.walk", Animation.LoopType.LOOP));
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.INDOMINUS_REX_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.INDOMINUS_REX_HURT.get();
    }
    ...
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.INDOMINUS_REX_DEATH.get();
    }
}