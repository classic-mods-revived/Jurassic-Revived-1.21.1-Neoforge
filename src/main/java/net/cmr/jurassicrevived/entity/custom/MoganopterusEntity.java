...
public class MoganopterusEntity extends Animal implements GeoEntity, FlyingAnimal {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(MoganopterusEntity.class, EntityDataSerializers.INT);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(8, new WaterAvoidingRandomStrollGoal(this, 1.0, 20) {
            ...
            protected Vec3 getPosition() {
                ...
                if (MoganopterusEntity.this.onGround()) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.MOGANOPTERUS.get().create(pLevel);
        if (child instanceof MoganopterusEntity baby) {
            MoganopterusVariant randomVariant = Util.getRandom(MoganopterusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            ...
            if (state.isMoving())
                return state.setAndContinue(MoganopterusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.moganopterus.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.moganopterus.walk", Animation.LoopType.LOOP));
        ...
        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.moganopterus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.moganopterus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public MoganopterusVariant getVariant() {
        return MoganopterusVariant.byId(this.getTypeVariant() & 255);
    }
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof MoganopterusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        MoganopterusVariant variant = Util.getRandom(MoganopterusVariant.values(), this.random);
