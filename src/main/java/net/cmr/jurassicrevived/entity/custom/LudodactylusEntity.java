...
public class LudodactylusEntity extends Animal implements GeoEntity, FlyingAnimal {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(LudodactylusEntity.class, EntityDataSerializers.INT);
    ...
    public LudodactylusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return LudodactylusEntity.this.isBaby() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(7, new WaterAvoidingRandomStrollGoal(this, 1.0) {
            @Override
            public boolean canUse() {
                return LudodactylusEntity.this.onGround() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(8, new WaterAvoidingRandomStrollGoal(this, 1.0, 20) {
            @Override
            public boolean canUse() {
                boolean isFlying = !LudodactylusEntity.this.onGround();
                // If flying, keep flying. If on ground, small chance (1/400 ticks) to take off.
                return (isFlying || LudodactylusEntity.this.getRandom().nextInt(400) == 0) && super.canUse();
            }

            @Override
            protected Vec3 getPosition() {
                Vec3 pos = LudodactylusEntity.this.position();
                RandomSource random = LudodactylusEntity.this.getRandom();
                ...
                int groundY = LudodactylusEntity.this.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int)x, (int)z);
                double y;

                if (LudodactylusEntity.this.onGround()) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.LUDODACTYLUS.get().create(pLevel);
        if (child instanceof LudodactylusEntity baby) {
            LudodactylusVariant randomVariant = Util.getRandom(LudodactylusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (!LudodactylusEntity.this.onGround()) {
                return state.setAndContinue(RawAnimation.begin().then("anim.ludodactylus.fly", Animation.LoopType.LOOP));
            }

            if (state.isMoving())
                return state.setAndContinue(LudodactylusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.ludodactylus.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.ludodactylus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.ludodactylus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.ludodactylus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.ludodactylus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.LUDODACTYLUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public LudodactylusVariant getVariant() {
        return LudodactylusVariant.byId(this.getTypeVariant() & 255);
    }
    public int getTypeVariant() {
        return this.entityData.get(DATA_ID_TYPE_VARIANT);
    }

    private void setVariant(LudodactylusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof LudodactylusEntity that)) return false;
        return this.getVariant() != that.getVariant();
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        LudodactylusVariant variant = Util.getRandom(LudodactylusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.LUDODACTYLUS_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.LUDODACTYLUS_DEATH.get();
    }
}