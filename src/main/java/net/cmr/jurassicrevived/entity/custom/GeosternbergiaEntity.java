...
public class GeosternbergiaEntity extends Animal implements GeoEntity, FlyingAnimal {
    ...
    public GeosternbergiaEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return GeosternbergiaEntity.this.isBaby() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(7, new WaterAvoidingRandomStrollGoal(this, 1.0) {
            @Override
            public boolean canUse() {
                return GeosternbergiaEntity.this.onGround() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(8, new WaterAvoidingRandomStrollGoal(this, 1.0, 20) {
            @Override
            public boolean canUse() {
                boolean isFlying = !GeosternbergiaEntity.this.onGround();
                // If flying, keep flying. If on ground, small chance (1/400 ticks) to take off.
                return (isFlying || GeosternbergiaEntity.this.getRandom().nextInt(400) == 0) && super.canUse();
            }

            @Override
            protected Vec3 getPosition() {
                Vec3 pos = GeosternbergiaEntity.this.position();
                RandomSource random = GeosternbergiaEntity.this.getRandom();
                ...
                int groundY = GeosternbergiaEntity.this.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int)x, (int)z);
                double y;

                if (GeosternbergiaEntity.this.onGround()) {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (!GeosternbergiaEntity.this.onGround()) {
                return state.setAndContinue(RawAnimation.begin().then("anim.geosternbergia.fly", Animation.LoopType.LOOP));
            }

            if (state.isMoving())
                return state.setAndContinue(GeosternbergiaEntity.this.isSprinting() ? RawAnimation.begin().then("anim.geosternbergia.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.geosternbergia.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.geosternbergia.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.geosternbergia.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.geosternbergia.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.GEOSTERNBERGIA_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    private void setVariant(GeosternbergiaVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof GeosternbergiaEntity that)) return false;
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.GEOSTERNBERGIA_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.GEOSTERNBERGIA_DEATH.get();
    }
}