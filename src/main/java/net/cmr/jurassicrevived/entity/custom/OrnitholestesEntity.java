...
public class OrnitholestesEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(OrnitholestesEntity.class, EntityDataSerializers.INT);
    ...
    public OrnitholestesEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        ...
        public boolean canUse() {
            return OrnitholestesEntity.this.isBaby() && super.canUse();
        }
    });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.ORNITHOLESTES.get().create(pLevel);
        if (child instanceof OrnitholestesEntity baby) {
            OrnitholestesVariant randomVariant = Util.getRandom(OrnitholestesVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(OrnitholestesEntity.this.isSprinting() ? RawAnimation.begin().then("anim.ornitholestes.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.ornitholestes.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.ornitholestes.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.ornitholestes.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.ornitholestes.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.ORNITHOLESTES_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public OrnitholestesVariant getVariant() {
        return OrnitholestesVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(OrnitholestesVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof OrnitholestesEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        OrnitholestesVariant variant = Util.getRandom(OrnitholestesVariant.values(), this.random);
