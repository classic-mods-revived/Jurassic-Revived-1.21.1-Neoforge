...
public class AllosaurusEntity extends Animal implements GeoEntity {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.ALLOSAURUS.get().create(pLevel);
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.ALLOSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public AllosaurusVariant getVariant() {
        return AllosaurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(AllosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof AllosaurusEntity that)) return false;
