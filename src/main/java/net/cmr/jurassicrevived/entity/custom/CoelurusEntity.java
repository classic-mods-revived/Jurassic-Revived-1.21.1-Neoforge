...
public class CoelurusEntity extends Animal implements GeoEntity {
    ...
    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(CoelurusEntity.class, EntityDataSerializers.INT);
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.COELURUS.get().create(pLevel);
        if (child instanceof CoelurusEntity baby) {
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.COELURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public CoelurusVariant getVariant() {
        return CoelurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(CoelurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof CoelurusEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        CoelurusVariant variant = Util.getRandom(CoelurusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.COELURUS_HURT.get();
    }
