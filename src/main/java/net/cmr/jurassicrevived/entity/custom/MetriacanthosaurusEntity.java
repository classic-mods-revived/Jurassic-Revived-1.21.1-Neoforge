...
public class MetriacanthosaurusEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(MetriacanthosaurusEntity.class, EntityDataSerializers.INT);
    ...
    public MetriacanthosaurusEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        ...
        public boolean canUse() {
            return MetriacanthosaurusEntity.this.isBaby() && super.canUse();
        }
    });
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.METRIACANTHOSAURUS.get().create(pLevel);
        if (child instanceof MetriacanthosaurusEntity baby) {
            MetriacanthosaurusVariant randomVariant = Util.getRandom(MetriacanthosaurusVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(MetriacanthosaurusEntity.this.isSprinting() ? RawAnimation.begin().then("anim.metriacanthosaurus.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.metriacanthosaurus.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.metriacanthosaurus.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.metriacanthosaurus.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.metriacanthosaurus.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.METRIACANTHOSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public MetriacanthosaurusVariant getVariant() {
        return MetriacanthosaurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(MetriacanthosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof MetriacanthosaurusEntity that)) return false;
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData) {
        MetriacanthosaurusVariant variant = Util.getRandom(MetriacanthosaurusVariant.values(), this.random);
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.METRIACANTHOSAURUS_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.METRIACANTHOSAURUS_DEATH.get();
    }
}