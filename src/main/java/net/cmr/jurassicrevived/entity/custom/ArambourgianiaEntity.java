...
public class ArambourgianiaEntity extends Animal implements GeoEntity, FlyingAnimal {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.ARAMBOURGIANIA.get().create(pLevel);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            ...
            if (state.isMoving())
                return state.setAndContinue(ArambourgianiaEntity.this.isSprinting() ? RawAnimation.begin().then("anim.arambourgiania.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.arambourgiania.walk", Animation.LoopType.LOOP));
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.ARAMBOURGIANIA_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    private void setVariant(ArambourgianiaVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof ArambourgianiaEntity that)) return false;
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.ARAMBOURGIANIA_HURT.get();
    }
