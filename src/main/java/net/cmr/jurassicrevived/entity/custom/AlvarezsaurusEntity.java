...
public class AlvarezsaurusEntity extends Animal implements GeoEntity {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.ALVAREZSAURUS.get().create(pLevel);
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.ALVAREZSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public AlvarezsaurusVariant getVariant() {
        return AlvarezsaurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(AlvarezsaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof AlvarezsaurusEntity that)) return false;
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.ALVAREZSAURUS_HURT.get();
    }
