...
public class AnkylosaurusEntity extends Animal implements GeoEntity {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.ANKYLOSAURUS.get().create(pLevel);
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.ANKYLOSAURUS_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public AnkylosaurusVariant getVariant() {
        return AnkylosaurusVariant.byId(this.getTypeVariant() & 255);
    }

    private void setVariant(AnkylosaurusVariant variant) {
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof AnkylosaurusEntity that)) return false;
