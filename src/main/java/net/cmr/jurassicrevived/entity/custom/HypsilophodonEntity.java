...
public class HypsilophodonEntity extends Animal implements GeoEntity {
    private AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> VARIANT =
            SynchedEntityData.defineId(HypsilophodonEntity.class, EntityDataSerializers.INT);
    ...
    public HypsilophodonEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.HYPSILOPHODON.get().create(pLevel);
        if (child instanceof HypsilophodonEntity baby) {
            HypsilophodonVariant randomVariant = Util.getRandom(HypsilophodonVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (state.isMoving())
                return state.setAndContinue(HypsilophodonEntity.this.isSprinting() ? RawAnimation.begin().then("anim.hypsilophodon.run", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.hypsilophodon.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.hypsilophodon.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.hypsilophodon.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.hypsilophodon.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.HYPSILOPHODON_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof HypsilophodonEntity that)) return false;
    ...
    protected @Nullable SoundEvent getHurtSound(DamageSource damageSource) {
        return ModSounds.HYPSILOPHODON_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.HYPSILOPHODON_DEATH.get();
    }
}