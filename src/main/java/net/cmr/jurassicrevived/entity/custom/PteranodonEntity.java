...
import net.cmr.jurassicrevived.entity.client.PteranodonVariant;
...
public class PteranodonEntity extends Animal implements GeoEntity, FlyingAnimal {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT =
            SynchedEntityData.defineId(PteranodonEntity.class, EntityDataSerializers.INT);
    ...
    public PteranodonEntity(EntityType<? extends Animal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new SprintingPanicGoal(this, 1.15) {
            ...
            public boolean canUse() {
                return PteranodonEntity.this.isBaby() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(7, new WaterAvoidingRandomStrollGoal(this, 1.0) {
            @Override
            public boolean canUse() {
                return PteranodonEntity.this.onGround() && super.canUse();
            }
        });
        ...
        this.goalSelector.addGoal(8, new WaterAvoidingRandomStrollGoal(this, 1.0, 20) {
            @Override
            public boolean canUse() {
                boolean isFlying = !PteranodonEntity.this.onGround();
                // If flying, keep flying. If on ground, small chance (1/400 ticks) to take off.
                return (isFlying || PteranodonEntity.this.getRandom().nextInt(400) == 0) && super.canUse();
            }

            @Override
            protected Vec3 getPosition() {
                Vec3 pos = PteranodonEntity.this.position();
                ...
                int groundY = PteranodonEntity.this.level().getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int)x, (int)z);
                double y;
    ...
    public AgeableMob getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        AgeableMob child = ModEntities.PTERANODON.get().create(pLevel);
        if (child instanceof PteranodonEntity baby) {
            PteranodonVariant randomVariant = Util.getRandom(PteranodonVariant.values(), this.random);
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "Walk/Run/Idle", state -> {
            if (!PteranodonEntity.this.onGround()) {
                return state.setAndContinue(RawAnimation.begin().then("anim.pteranodon.fly", Animation.LoopType.LOOP));
            }

            if (state.isMoving())
                return state.setAndContinue(PteranodonEntity.this.isSprinting() ? RawAnimation.begin().then("anim.pteranodon.walk", Animation.LoopType.LOOP) : RawAnimation.begin().then("anim.pteranodon.walk", Animation.LoopType.LOOP));

            return state.setAndContinue(RawAnimation.begin().then("anim.pteranodon.idle", Animation.LoopType.LOOP));
        }));

        controllers.add(new AnimationController<>(this, "attackController", state -> PlayState.STOP)
                .triggerableAnim("attack", RawAnimation.begin().then("anim.pteranodon.attack", Animation.LoopType.PLAY_ONCE)));

        controllers.add(new AnimationController<>(this, "mouthController", state -> PlayState.STOP)
                .triggerableAnim("mouth", RawAnimation.begin().then("anim.pteranodon.mouth", Animation.LoopType.PLAY_ONCE)));
    }
    ...
    public void tick() {
        ...
        if (!level().isClientSide) {
            if (mouthAnimCooldown > 0) {
                ...
                this.playSound(ModSounds.PTERANODON_CALL.get(), 1.0F, 1.0F);
                // 30s–60s in ticks
    ...
    public boolean canMate(Animal other) {
        ...
        if (!(other instanceof PteranodonEntity that)) return false;
